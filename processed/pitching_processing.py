...
data = pd.read_sql("SELECT * FROM raw_pitching", connection)
...
data.to_sql("processed_pitching", connection, if_exists="replace", index=False)

connection.close()

print(f"Table 'processed_pitching' replaced with new data. {len(data)} records added.")