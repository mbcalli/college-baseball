...
data = pd.read_sql("SELECT * FROM raw_batting", connection)
...
data.to_sql("processed_batting", connection, if_exists="replace", index=False)

connection.close()

print(f"Table 'processed_batting' replaced with new data. {len(data)} records added.")